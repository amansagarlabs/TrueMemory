...
import { ArrowLeft, ArrowRight, FolderKanban, Plus, Trash2 } from "lucide-react";
...
export default function WorkspacesPage() {
  ...
  function removeWorkspace(id: string) {
    // Workspace deletion is intentionally not local-only; the backend currently
    // exposes no destructive workspace endpoint.
    ...
    <div className="theme-surface-page workspaces-page min-h-screen bg-[var(--chat-background)] text-[var(--chat-foreground)]">
      <div className="mx-auto max-w-[1280px] px-5 py-6 sm:px-8 lg:px-10 lg:py-9">
        <header className="flex items-center justify-between gap-4">
          ...
          <Link href="/" className="flex items-center gap-2.5 text-[17px] font-semibold tracking-[-0.03em]"><span aria-hidden="true" className="size-6 rounded-full bg-[linear-gradient(135deg,#fff5a5,#f6e66c_42%,#f27a28)]" />kontext</Link>
        </header>
        ...
        <section className="relative mt-7 overflow-hidden rounded-[24px] border border-white/10 bg-[#0c0a08] p-6 sm:p-8 lg:p-10">
          ...
          <div className="relative z-10 max-w-xl">
            <p className="font-mono text-[10px] uppercase tracking-[0.17em] text-[#f6e879]">Context boundaries</p>
            <h1 className="mt-3 font-heading text-4xl font-medium tracking-[-0.055em] sm:text-5xl">Your workspaces.</h1>
            <p className="mt-4 max-w-lg text-sm leading-7 text-white/45">Separate sources, memory, and active work by project. Workspace preferences are currently stored on this device.</p>
          </div>
        </section>
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[0.72fr_1.28fr]">
          <section className="h-fit rounded-[20px] border border-white/[0.08] bg-[#10100f] p-5 sm:p-6">
            <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-[#f6e879]">New workspace</p>
            <form onSubmit={createWorkspace} className="mt-5">
              <label className="text-sm font-medium text-white/70" htmlFor="workspace-name">Workspace name</label>
          ...
          <section className="overflow-hidden rounded-[20px] border border-white/[0.08] bg-[#10100f]">
            <div className="flex items-center justify-between border-b border-white/[0.08] px-5 py-4">
              <div className="flex items-center gap-2 text-sm font-semibold text-white/75"><FolderKanban aria-hidden="true" className="size-4 text-[#f6e879]" />Managed workspaces</div>
            ...
            {loading ? (
              ...
              <div className="px-6 py-16 text-center"><p className="text-sm font-medium text-white/60">No workspace yet</p><p className="mt-2 text-xs text-white/30">Create your first context boundary using the form.</p></div>
            ) : (
              <div className="divide-y divide-white/[0.07]">
                {workspaces.map((workspace) => (
                  <div key={workspace.id} className="flex items-center gap-4 px-5 py-4 transition hover:bg-white/[0.025]">
                    ...
                    <button type="button" aria-label={`Delete ${workspace.name}`} onClick={() => removeWorkspace(workspace.id)} className="grid size-9 place-items-center rounded-lg text-white/25 transition hover:bg-red-400/[0.08] hover:text-red-300"><Trash2 aria-hidden="true" className="size-4" /></button>
                  </div>
                ))}
              </div>
            )}
