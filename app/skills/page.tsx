      ...
      <main className="min-h-screen bg-[var(--chat-background)] px-4 py-8 text-[var(--chat-foreground)] sm:px-8">
        <div className="mx-auto max-w-6xl">
          <header className="flex flex-col gap-5 border-b border-[var(--chat-border)] pb-7 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="flex items-center gap-2 font-mono text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--chat-accent)]"><Sparkles className="size-3.5" /> Kontext Skills</p>
          ...
          <section className="mt-10 grid gap-3 md:grid-cols-3">{["Trending skills", "Recommended for you", "Official Kontext skills"].map((title) => <div key={title} className="rounded-2xl border border-[var(--chat-border)] p-4"><h2 className="font-semibold">{title}</h2><p className="mt-2 text-sm leading-5 text-[var(--chat-muted-foreground)]">New capabilities will appear here as registry providers and usage signals are connected.</p></div>)}</section>
        </div>
      </main>
