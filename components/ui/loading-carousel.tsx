...
const defaultTips: Tip[] = [
  {
    text: "Reading your document and preserving its structure.",
  },
  {
    text: "Creating searchable chunks for grounded retrieval.",
  },
  {
    text: "Connecting this source to your workspace memory.",
  },
  {
    text: "Preparing source-aware answers for this conversation.",
  },
]
...
const textVariants: Variants = {
  ...
  visible: { opacity: 1, y: 0, transition: { delay: 0.08, duration: 0.22 } },
}
...
export function LoadingCarousel({
  ...
  useEffect(() => {
    ...
    const onSelect = () => {
      ...
      setDirection(newIndex - currentRef.current)
            ...
            <AnimatePresence initial={false} custom={direction}>
              {(displayTips || []).map((tip) => (
                <CarouselItem key={getTipKey(tip)} className="min-w-0">
                  ...
                  >
                    {tip.visual ? (
                      <div className="grid size-full place-items-center bg-[radial-gradient(circle_at_50%_28%,rgba(230,125,43,0.24),transparent_46%),linear-gradient(145deg,#fffdfa,#f1e8dc)] text-[#b6561b] dark:bg-[radial-gradient(circle_at_50%_28%,rgba(230,125,43,0.18),transparent_46%),linear-gradient(145deg,#11130f,#191d18)] dark:text-[#e67d2b]">
                        {tip.visual}
                      </div>
                    ) : tip.image ? (
                      <Image
                      ...
                      <div className="size-full bg-[#f1e8dc] [background-image:radial-gradient(rgba(230,125,43,0.55)_0.8px,transparent_0.95px)] [background-size:6px_6px] dark:bg-[#11130f] dark:[background-image:radial-gradient(rgba(230,125,43,0.42)_0.8px,transparent_0.95px)]" />
                    )}
                    {backgroundGradient && (
                ...
                <div className="min-w-0 max-w-full">
                  {displayTips[current]?.url ? (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="wrap-break-word block max-w-full rounded-sm text-sm font-medium leading-5 tracking-tight text-pretty focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-4 focus-visible:ring-offset-muted sm:text-base"
                    >
                      {displayTips[current]?.text}
                    </a>
                  ) : (
                    <span className="wrap-break-word block max-w-full text-sm font-medium leading-5 tracking-tight text-pretty sm:text-base">
                      {displayTips[current]?.text}
                    </span>
                  )}
                </div>
