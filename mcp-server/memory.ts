...
const API_URL = (process.env.TrueMemory_MEMORY_URL || "http://localhost:8010").replace(/\/$/, "");
const API_KEY = process.env.TrueMemory_API_KEY || "";
const ACCESS_TOKEN = process.env.TrueMemory_MEMORY_TOKEN || "";
...
async function call(path: string, init: RequestInit = {}): Promise<unknown> {
  ...
  if (!response.ok) throw new Error(`TrueMemory Memory ${response.status}: ${JSON.stringify(payload)}`);
  return payload;
}

server.tool(
  "memory_search",
  "Search authorized TrueMemory memories.",
...
server.tool(
  "memory_retrieve",
  "Retrieve authorized TrueMemory memories for context assembly.",
  { query: z.string().optional(), scope: z.string().optional(), limit: z.number().optional() },
  async ({ query = "", scope = "general", limit = 10 }) => ({
...
server.tool(
  "memory_store",
  "Store a user or agent memory in TrueMemory.",
...
server.tool(
  "memory_update",
  "Update an existing TrueMemory memory.",
...
server.tool(
  "memory_forget",
  "Forget an existing TrueMemory memory.",
  { id: z.string() },
  async ({ id }) => ({
